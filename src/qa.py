...
def ask(db, history, search_text, prompt_query=None, k=8, category=None):
    ...
    # Generation: send the prompt to the local Ollama server to generate an answer.
    # stream=False -> receive the complete response at once, without token streaming.
    try:
        ...
        if "response" in data:
            ...
            answer = f"[Ollama error]: {data['error']}"
        else:
            answer = f"[Unknown response]: {response.text[:200]}"
    except requests.exceptions.RequestException as e:
        # Server not running, connection dropped, etc. Swallow the exception and return it as a message.
        answer = f"[Ollama connection failed]: {e}"
